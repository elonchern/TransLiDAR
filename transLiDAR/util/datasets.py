...
class PairDataset(torch.utils.data.Dataset):
    def __init__(self, dataset):
        """
        Args:
            dataset: 输入的数据集，要求是通过 RangeMapFolder 获取的样本。
        """
        self.dataset = dataset
        self.pairs = []  # 存储配对后的索引对

        # 预处理数据集，按 name 分组并寻找配对样本
        name_to_samples = {}
        for i, sample in enumerate(dataset):
            name = sample['name']
            if name not in name_to_samples:
                name_to_samples[name] = []
            name_to_samples[name].append((i, sample['class']))

        # 查找配对，并检查约束条件
        for name, samples in name_to_samples.items():
            # 分别筛选 class=0 和 class=1 的样本
            class_0 = [s[0] for s in samples if s[1] == 0]
            class_1 = [s[0] for s in samples if s[1] == 1]

            # 检查每个 name 是否满足要求
            if len(class_0) != 1 or len(class_1) != 1:
                raise ValueError(
                    f"Error: The name '{name}' must have exactly one class=0 "
                    f"and one class=1 sample, but got {len(class_0)} class=0 "
                    f"and {len(class_1)} class=1 samples."
                )

            # 配对 (class=0, class=1)
            self.pairs.append((class_0[0], class_1[0]))
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: (sample_0, sample_1)，分别对应 class=0 和 class=1 的样本。
        """
        idx_0, idx_1 = self.pairs[index]
        sample_0 = self.dataset[idx_0]
        sample_1 = self.dataset[idx_1]
...
def npy_loader(path: str) -> np.ndarray:
    with open(path, "rb") as f:
        ...
        range_map = range_intensity_map[..., 0]
        label = range_intensity_map[..., 1]
        
    return range_map.astype(np.float32), label.astype(np.uint8) 
...
def build_kitti_upsampling_dataset(is_train, args):
    ...
    dataset_low_res = RangeMapFolder(root_low_res, transform = transform_low_res, loader= npy_loader, class_dir = True)
    dataset_high_res = RangeMapFolder(root_high_res, transform = transform_high_res, loader = npy_loader, class_dir = True)

    assert len(dataset_high_res) == len(dataset_low_res)
    dataset_concat = PairDataset(dataset_high_res)
    # dataset_concat = PairDataset(dataset_low_res, dataset_high_res)
    return dataset_concat
