...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (samples_low_res, samples_high_res) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        label_high = samples_high_res['label']
        mask_high = (label_high >= 1) & (label_high <= 8)  # mask 的形状将是 [B, 64, 256]
        mask_high = mask_high.int().unsqueeze(1)
        label_low = samples_low_res['label']
        mask_low = (label_low >= 1) & (label_low <= 8)  # mask 的形状将是 [B, 64, 256]
        mask_low = mask_low.int().unsqueeze(1)
...
def evaluate(data_loader, model, device, log_writer, args=None):
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        label_high = batch[1]['label']
        mask_high = (label_high >= 1) & (label_high <= 8)  # mask 的形状将是 [B, 64, 256]
        mask_high = mask_high.int().unsqueeze(1)
        label_low = batch[0]['label']
        mask_low = (label_low >=1) & (label_low <= 8)  # mask 的形状将是 [B, 64, 256]
        mask_low = mask_low.int().unsqueeze(1)
        ...
        if log_writer is not None:
            ...
            if args.dataset_select == "carla":
                ...
                diff = np.abs(images_high_res - images_low_res)
                diff = np.where(diff > 0.15, 0, 1)
                pred_img = diff*pred_img + images_low_res*(1-diff)
                pred_img = pred_img.astype(np.float32)


                # 3D Evaluation Metrics
                pcd_pred = img_to_pcd_kitti(pred_img, maximum_range= 80) # pred_img ->[64, 256]
                pcd_gt = img_to_pcd_kitti(images_high_res, maximum_range = 80)
                array_gt = img_to_pcd_kitti(images_low_res, maximum_range = 80)
                ...
                if args.save_pcd:
                    ...
                    if local_step % 10 == 0:
                        ...
                        point_cloud_gt = trimesh.PointCloud(
                            vertices=pcd_gt,
                            colors=pcd_gt_color)
                        point_cloud_array = trimesh.PointCloud(
                            vertices=array_gt,
                            colors=pcd_gt_color)
                        ...
                        # point_cloud_array.export(os.path.join(pcd_outputpath, f"array_{local_step}.ply")) 
    ...
    if log_writer is not None:
        ...
        print(f'Metrics/test_average_iou: {total_iou / global_step:.4f}')
        print(f'Metrics/test_average_cd: {total_cd / global_step:.4f}')
        print(f'Metrics/test_average_loss: {avg_loss:.4f}')
        print(f'Metrics/test_average_f1: {total_f1 / global_step:.4f}')
        print(f'Metrics/test_average_precision: {total_precision / global_step:.4f}')
        print(f'Metrics/test_average_recall: {total_recall / global_step:.4f}')
