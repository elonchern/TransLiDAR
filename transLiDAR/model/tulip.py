...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        x = self.padding(x) # [B, 1, 64, 256] ->  [B, 1, 64, 260]
        ...
        x = self.proj(x)
        x = rearrange(x, 'B C H W -> B H W C') # [B 96, 16, 64] ->  [B,16,64, 96] 
        x = self.norm(x)
        return x
...
class TULIP(nn.Module):
    ...
    def forward(self, x, target, mask_low,mask_high, eval = False, mc_drop = False):

        x = self.patch_embed(x) # [B, 1, 64, 256] -> [B, 16, 64, 96]
        x = self.pos_drop(x) # [B, 16, 64, 96]
        x_save = []
        for i, layer in enumerate(self.layers): # [B, 8, 32, 192] [B, 4, 16, 384] [B, 2, 8, 768]
            x_save.append(x)
            x = layer(x)
            
        x = self.first_patch_expanding(x) # [B, 4, 16, 384]


        for i, layer in enumerate(self.layers_up):
            x = torch.cat([x, x_save[len(x_save) - i - 2]], -1) #[B, 8, 32, 192] [B, 16, 64, 96]
            x = self.skip_connection_layers[i](x)
            x = layer(x)

        
        x = self.norm_up(x)  # [B, 16, 64, 96]
        

        if self.pixel_shuffle:
            x = rearrange(x, 'B H W C -> B C H W') # [B, 16, 64, 96] -> [B, 96, 16, 64]
        ...
        if mc_drop:
            ...
            low_loss, pixel_low_loss = self.forward_loss(x*mask_low, target*mask_high)
            high_loss, pixel_high_loss = self.forward_loss(x*(1-mask_low), target*(1-mask_high))

            total_loss = low_loss + high_loss
            pixel_loss = pixel_low_loss + pixel_high_loss
